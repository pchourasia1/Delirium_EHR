...
# ---- select top N features by |rho| with the target ----------------------
tcorr_all = df.apply(lambda c: c.corr(target, method="spearman")).dropna()
...
mask = np.triu(np.ones_like(corr, dtype=bool), k=0)   # k=0 drops the rho=1 diagonal
n = corr.shape[0]
...
sns.heatmap(
    ...
    cbar_kws={"shrink": 0.5, "aspect": 30, "pad": 0.02,
              "label": r"Spearman $\rho$"},
    ax=ax,
)
ax.set_title(f"Spearman correlation, top {TOP_N} features", fontsize=15, pad=12)
ax.set_xticklabels(ax.get_xticklabels(), rotation=90, fontsize=8)
ax.set_yticklabels(ax.get_yticklabels(), rotation=0, fontsize=8)
fig.savefig("ridge_spearman_correlation_heatmap.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("saved ridge_spearman_correlation_heatmap.png")
...
print(f"\nFeature pairs with |rho| >= 0.80: {len(redundant)}")
print(redundant.to_string(index=False) if len(redundant) else "  none")
redundant.to_csv("ridge_spearman_redundant_pairs.csv", index=False)
...
figt.tight_layout()
figt.savefig("ridge_spearman_correlation.png", dpi=300, bbox_inches="tight")
plt.close(figt)
print("saved ridge_spearman_correlation.png")